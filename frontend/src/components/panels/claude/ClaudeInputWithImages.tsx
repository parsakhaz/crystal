...
import { X, Send, Play, Terminal, ChevronRight, AtSign, Paperclip, Zap, Brain, Target, CheckCircle, Square, FileText, Loader2 } from 'lucide-react';
    ...
    <div className="h-full flex flex-col shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
      <div className="bg-surface-secondary flex flex-col flex-1 min-h-0">
        ...
        >
          ...
          {/* Minimal Action Bar */}
          <div className="flex items-center justify-between mt-2 gap-3 flex-shrink-0">
            {/* Left Section - Compact icon buttons */}
            <div className="flex items-center gap-1">
              ...
              {/* Subtle divider */}
              <div className="h-4 w-px bg-border-primary/50 mx-1" />
              ...
              {/* Extended Thinking - Icon toggle only */}
              <button
                ...
                title={`Extended Thinking: ${ultrathink ? 'On' : 'Off'}`}
              >
                <Brain className="w-4 h-4" />
              </button>

              {/* Compact Context - Icon only when available */}
              {handleCompactContext && hasConversationHistory && (
                <button
                  ...
                  title={contextCompacted
                    ? 'Context ready - will be added to next prompt'
                    : 'Compact context'
                  }
                >
              ...
              {activeSession.status === 'running' && handleCancelRequest ? (
                <button
                  ...
                  className="px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1.5
                    text-status-error hover:bg-status-error/10 transition-colors"
                  title="Cancel (Esc)"
                >
                ...
                >
                  ...
                  <span>{isSubmitting ? '...' : buttonConfig.text}</span>
                </button>
              )}
            </div>
          </div>
