...
export const useClaudePanel = (
  ...
  useEffect(() => {
    // Focus when transitioning from inactive to active, OR on first render if already active
    const isFirstRender = prevIsActiveRef.current === null;
    const becameActive = isActive && !prevIsActiveRef.current;

    if (isActive && (isFirstRender || becameActive)) {
